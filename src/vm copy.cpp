...
std::tuple<std::uint32_t, std::uint32_t> VirtualMachine::decodeRegisterAndImmediate(std::uint32_t operand) {
...
std::tuple<std::uint32_t, std::uint32_t> VirtualMachine::decodeTwoRegisters(std::uint32_t operand) {
...
std::tuple<std::uint32_t, std::uint32_t, std::uint32_t> VirtualMachine::decodeThreeRegisters(std::uint32_t operand) {
...
std::uint32_t VirtualMachine::encodeRegisterAndImmediate(std::uint32_t reg1, std::uint32_t immediate) {
    std::uint32_t res = (reg1 & 0xF) << 32 | (immediate & 0xFFFFFFFF);
    // std::cout << "encodeRegisterAndImmediate val: "<<res <<std::endl;
    return res;
}

std::uint32_t VirtualMachine::encodeTwoRegisters(std::uint32_t first, std::uint32_t second) {
    // 각 레지스터 인덱스는 32비트에 저장
    // 32 * 2 = 32비트, 따라서 std::uint32_t 내에 모두 저장 가능

    return (static_cast<std::uint32_t>(first) << 32) |
           static_cast<std::uint32_t>(second);
}

std::uint32_t VirtualMachine::encodeThreeRegisters(std::uint32_t first, std::uint32_t second, std::uint32_t third) {
    // 각 레지스터 인덱스는 21비트에 저장
    // 21 * 3 = 63비트, 따라서 std::uint32_t 내에 모두 저장 가능

    return (static_cast<std::uint32_t>(first) << 42) |
           (static_cast<std::uint32_t>(second) << 21) |
